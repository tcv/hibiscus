...
data_dir = '/home/tcv/lustre/data_arrays_lim/'
out_dir = '/home/tcv/lustre/processed_data_sept/'
...
diff_time = loadtxt(cal_dir+'Cal_time_subavg.txt')
Temp_gain = loadtxt(cal_dir+'TempGain_subavg.txt')
Temp_gainZ = loadtxt(cal_dir+'TempGainZ_subavg.txt')
GainR = loadtxt(cal_dir+'Real_Gain_subavg.txt')
GainX = loadtxt(cal_dir+'Imag_Gain_subavg.txt')
InR = loadtxt(cal_dir+'Real_In_subavg.txt')
InX = loadtxt(cal_dir+'Imag_In_subavg.txt')
VnR = loadtxt(cal_dir+'Real_Vn_subavg.txt')
VnX = loadtxt(cal_dir+'Imag_Vn_subavg.txt')
new_freq = loadtxt(cal_dir+'Cal_freq_subavg.txt')
cal_volt = loadtxt(cal_dir+'Cal_volt_subavg.txt')
cal_temp = loadtxt(cal_dir+'Cal_temp_subavg.txt')
print diff_time
...
##### Gain Calibration 
        ...
        for i in range(0,len(data_eff_cal_db)):
            ...
            TskyZ = zeros(len(data_eff_cal_db[0]))
            cal_ind = 0
#            print ant_time[i]
...
#               Tsky[j] = TGain_s*Psky
        ...
        ant_eff_noise_corrZ = array(ant_eff_noise_corrZ)
...
#####Write unrebinned data to file:
        for t in range(0,len(ant_eff_noise_corr)):
            full_ant.append(ant_eff_noise_corr[t])
            full_antZ.append(ant_eff_noise_corrZ[t])
            full_time.append(ant_time[t])
            full_volt.append(ant_volt[t])
            full_temp.append(ant_temp[t])
        full_freq = new_freq

savetxt(out_dir+date+'_processed_data_subavg.txt',full_ant,delimiter=' ')
savetxt(out_dir+date+'_processed_dataZ_subavg.txt',full_antZ,delimiter=' ')
savetxt(out_dir+date+'_processed_time_subavg.txt',full_time,delimiter=' ')
savetxt(out_dir+date+'_processed_volt_subavg.txt',full_volt,delimiter=' ')
savetxt(out_dir+date+'_processed_temp_subavg.txt',full_volt,delimiter=' ')
savetxt(out_dir+date+'_processed_freq_subavg.txt',full_freq,delimiter=' ')
